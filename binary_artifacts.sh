echo Refreshing binaries from github releases [https://github.com/safewallets/safecoin]
echo =========================================
...
echo
echo =========================================
echo Step: Downloading latest LINUX safecoin binaries from github
curl -s https://api.github.com/repos/FairExchange/safecoin/releases \
  | grep browser_download_url \
  | grep linux_master \
  | cut -d '"' -f 4 \
  | wget -qi -
tar xvfz safecoin_linux_master.tar.gz
mv -Tf src bins_linux/
echo =========================================
echo Step: Downloading latest WIN safecoin binaries from github
curl -s https://api.github.com/repos/FairExchange/safecoin/releases \
  | grep browser_download_url \
  | grep win64_master \
  | cut -d '"' -f 4 \
  | wget -qi -
unzip safecoin_win64_master.zip
mv safecoin_win_master bins_win/
cd ..
echo =========================================
echo
pwd
...
echo Moving OSX safecoin bins to assets/bin/osx
wget https://supernetorg.bintray.com/binaries/safe_osx_bins.zip
checksum=`shasum -a 256 safe_osx_bins.zip | awk '{ print $1 }'`
if [ "$checksum" = "4bb33149e4322d6d4a4c9ea41c3baa33ce125c4f2ccb7e7336fcd4f295ea5351" ]; then
    echo "Checksum is correct."
    unzip safe_osx_bins.zip
    cp -rvf safe_osx_bins/* assets/bin/osx/.
  else
...
chmod +x assets/bins_linux/safecoin*
echo Moving Linux safecoin bins to assets/bin
