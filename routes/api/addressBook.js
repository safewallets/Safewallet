...
module.exports = (api) => {
  ...
  api.post('/addressbook', async (req, res, next) => {
    ...
    if (api.checkToken(req.body.token)) {
      fs.writeFile(`${api.safewalletDir}/shepherd/addressBook.json`, JSON.stringify(data), (err) => {
        if (err) {
  ...
  api.get('/addressbook', (req, res, next) => {
    if (api.checkToken(req.query.token)) {
      if (fs.existsSync(`${api.safewalletDir}/shepherd/addressBook.json`)) {
        fs.readFile(`${api.safewalletDir}/shepherd/addressBook.json`, 'utf8', (err, data) => {
          if (err) {
            const retObj = {
              msg: 'error',
              result: err,
